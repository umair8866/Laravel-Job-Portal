            ...
            <div class="col-md-4 col-lg-3 sidebar mb-4">
                <form action="" name="searchForm" id="searchForm">
                    <div class="card border-0 shadow p-4">
                        ...
                        <a href="{{ route('jobs') }}" class="btn btn-success mt-3">Reset Filter</a>
                    </div>
                </form>
            </div>
                            ...
                            @if ($vacancies->isNotEmpty())
                                ...
                                <div class="col-md-4">
                                    <div class="card border-0 p-3 shadow mb-4">
                                        <div class="card-body">
                                            <h3 class="border-0 fs-5 pb-2 mb-0">{{ $vacancy->title }}</h3>
                                            <p>{{ Str::words($vacancy->description, $words=3, '...' ) }}</p>
                                            <div class="bg-light p-3 border">
                                                <p class="mb-0">
                                                    <span class="fw-bolder"><i class="fa fa-map-marker"></i></span>
                                                    <span class="ps-1">{{ $vacancy->location }}</span>
                                                </p>
                                                <p class="mb-0">
                                                    <span class="fw-bolder"><i class="fa fa-clock-o"></i></span>
                                                    <span class="ps-1">{{ $vacancy->vacancy_count }}</span>
                                                </p>
                                                @if(!is_null($vacancy->salary))
                                                <p class="mb-0">
                                                    <span class="fw-bolder"><i class="fa fa-usd"></i></span>
                                                    <span class="ps-1">{{ $vacancy->salary }}</span>
                                                </p>
                                                @endif 
                                            </div>
    
                                            <div class="d-grid mt-3">
                                                <a href="{{ route('jobDetail',$vacancy->id) }}" class="btn btn-primary btn-lg">Details</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>  
